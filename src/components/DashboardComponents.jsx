...
import { ECostCategoryTrend, ECostCompanyBars, ECostCompositionBars, ECostDetailBars, ECostItemTrend, ECostTrend, EProfitTrend, EProjectRevenueCharts, ERevenueAnalysisBars, ERevenueTrend } from '../charts'
...
export function CostCategoryPopup({ visible, value, options, onChange, onClose }) {
  return <Popup visible={visible} position="bottom" bodyClassName="organization-popup-body" onMaskClick={onClose} onClose={onClose}>
    <section className="organization-sheet cost-category-sheet" role="dialog" aria-label="科目选择">
      <div className="organization-sheet-header"><h2>科目</h2><button type="button" aria-label="关闭科目选择" onClick={onClose}><img src="/ZSRoad_mobile/assets/figma-organization-close.svg" alt="" /></button></div>
      <div className="organization-list">{options.map(name => <button type="button" className="organization-option" key={name} onClick={() => { onChange(name); onClose() }}><span>{name}</span>{value === name && <img src="/ZSRoad_mobile/assets/figma-organization-selected.svg" alt="" />}</button>)}</div>
    </section>
  </Popup>
}
...
export function RevenueDetailPage({ rows, sort, onSort, period, onPeriodChange, formatNumber }) {
  ...
  const chartPeriod = period === '年' ? '年累计' : '当月'
  return <div className="revenue-detail-page"><FactorPopup visible={factorOpen} value={factorName} options={factorNames} onChange={name => setFactorIndex(factorNames.indexOf(name))} onClose={() => setFactorOpen(false)}/><div className="revenue-meta"><span>数据更新时间：<em>2026-04-01</em></span><span>单位：<b>万元</b></span><div className="switch ranking-period-switch"><button className={period === '月' ? 'on' : ''} onClick={() => onPeriodChange('月')}>当月</button><button className={period === '年' ? 'on' : ''} onClick={() => onPeriodChange('年')}>年累计</button></div></div><ERevenueAnalysisBars title="各公司通行费收入" names={names} period={chartPeriod} series={[{ name: '通行费收入', data: [100,140,230,100,130,120,180,170,160], color: '#4E86EA' }, { name: '通行费收入(去年)', data: [150,100,200,140,100,142,120,126,130], color: '#B8B8B8' }]}/><ERevenueAnalysisBars title="各公司折算全程混合车流量" names={names} unit="万辆" period={chartPeriod} stacked series={[{ name: '客车流量', data: [100,140,230,100,130,120,180,170,160], color: '#B179F1' }, { name: '客车流量(去年)', data: [150,100,200,140,100,142,120,126,130], color: '#C9C9C9' }, { name: '货车流量', data: [250,240,430,240,300,260,300,295,290], color: '#61B8C0' }, { name: '货车流量(去年)', data: [310,295,530,300,280,325,375,365,370], color: '#C9C9C9' }]}/><ERevenueAnalysisBars title="影响因素" headerRight={factorSelector} names={names} period={chartPeriod} series={factorSeries}/><section className="revenue-detail-ranking"><Heading title="各公司数据明细" unit={false}/><div className="revenue-detail-table"><div className="revenue-detail-table-head">{detailHeaders.map((header, index) => <button type="button" key={index} onClick={() => index > 0 && setDetailSort(current => ({ index, direction: current.index === index && current.direction === "desc" ? "asc" : "desc" }))}>{header}{index > 0 && <i className={"detail-sort-mark " + (detailSort.index === index ? "active-" + detailSort.direction : "")}>◆</i>}</button>)}</div>{sortedDetailRows.map(row => <div className="revenue-detail-table-row" key={row[0]}>{row.map((value, index) => index === 0 ? <a key={index}>{value}</a> : <span key={index} className={String(value).startsWith('+') ? 'red' : String(value).startsWith('-') ? 'green' : ''}>{value}</span>)}</div>)}</div></section></div>
}
...
export function CostPage({ rows, sort, onSort, period, onPeriodChange, formatNumber, isProjectCompany = false }) {
  ...
  useEffect(() => {
    const label = document.querySelector('.cost-detail-category-switch span')
    if (!label) return undefined
    const open = () => setDetailCategoryOpen(true)
  ...
  const [trendCost, setTrendCost] = useState(null)
  ...
  const projectCostComposition = isProjectCompany ? <section className="cost-section project-cost-composition"><Heading title="成本构成" unit={false}/><div className="project-cost-period"><div className="project-cost-donut-row"><div className="cost-donut project-donut-current"><strong>当期</strong></div><div className="project-cost-legend"><div className="project-cost-legend-head"><span>总成本</span><span>占比</span></div>{costItems.map(([name], index) => <div className="project-cost-legend-row" key={name}><i className={`project-cost-dot project-cost-dot-${index + 1}`}/><span>{name}</span><b>6,000.00</b><em>35.09%</em></div>)}</div></div><div className="project-cost-donut-row"><div className="cost-donut project-donut-previous"><strong>去年同期</strong></div><div className="project-cost-legend"><div className="project-cost-legend-head"><span>总成本</span><span>占比</span></div>{costItems.map(([name], index) => <div className="project-cost-legend-row" key={name}><i className={`project-cost-dot project-cost-dot-prev-${index + 1}`}/><span>{name}</span><b>6,000.00</b><em>35.09%</em></div>)}</div></div></div></section> : null
  const detailPrimaryItems = [['人工成本', '2,568.00', '32.16%', '+1.23%'], ...costItems]
  const trendSection = <><CostCategoryPopup visible={detailCategoryOpen} value={detailCategory} options={detailCategories} onChange={name => setDetailCategoryIndex(detailCategories.indexOf(name))} onClose={() => setDetailCategoryOpen(false)}/>{isProjectCompany && trendCost && <div className="cost-trend-modal-backdrop" role="presentation" onClick={() => setTrendCost(null)}><section className="cost-trend-modal" role="dialog" aria-modal="true" aria-label={`${trendCost}趋势`} onClick={event => event.stopPropagation()}><header><h2>{trendCost}趋势</h2><button type="button" aria-label="关闭科目趋势" onClick={() => setTrendCost(null)}><X size={22}/></button></header><ECostItemTrend category={trendCost}/></section></div>}<section className="cost-section"><Heading title="成本趋势" unit={false}/><ECostTrend period={period === '年' ? '年累计' : '当月'}/></section>{projectCostComposition}</>
  return <div className="cost-page">
    <div className="cost-meta"><span>单位：<b>万元</b></span><span>数据更新时间：<em>2026-04-01</em></span><div className="switch cost-period-switch"><button type="button" className={period === '月' ? 'on' : ''} onClick={() => onPeriodChange('月')}>当月</button><button type="button" className={period === '年' ? 'on' : ''} onClick={() => onPeriodChange('年')}>年累计</button></div></div>
    {trendSection}
    <section className={`cost-section cost-composition-section ${selectedCost ? 'is-detail' : ''}`}><Heading title="经营成本构成" unit={false}>{selectedCost && <button type="button" className="cost-composition-back" onClick={() => setSelectedCost(null)}>‹ 返回</button>}</Heading><div className="cost-composition"><div className="cost-donut" style={donutStyle}><strong>{compositionTotal}<small>万元</small></strong></div><div className="cost-legend"><div className="cost-legend-head"><span>科目</span><span>成本金额</span><span>占比</span><span>同比</span></div>{compositionItems.map(([name, value, ratio, yoy], index) => <button type="button" className="cost-legend-row" key={name} onClick={() => !selectedCost && name === '道路养护费用' && setSelectedCost(name)}><i className={`cost-dot cost-dot-${index + 1}`}/><span>{name}</span><b>{value}</b><small>{ratio}</small><ChangeValue value={yoy}/></button>)}</div></div></section>
    <section className="cost-section"><Heading title="项目公司成本对比" unit={false}/><ECompanyBars period={period === '年' ? '年累计' : '当月'}/></section>
    <section className="cost-section"><Heading title="项目公司成本构成分析" unit={false}/><ECompanyBars composition period={period === '年' ? '年累计' : '当月'}/></section>
    <section className="cost-section cost-category-trend"><Heading title="各科目成本趋势分析" unit={false}/><ECostCategoryTrend/></section>
    <section className="cost-section cost-detail-section"><Heading title="成本构成明细" unit={false}><div className="cost-detail-category-switch"><button type="button" aria-label="上一个科目" disabled={detailCategoryIndex === 0} onClick={() => setDetailCategoryIndex(index => Math.max(0, index - 1))}>◀</button><span>{detailCategory}</span><button type="button" aria-label="下一个科目" disabled={detailCategoryIndex === detailCategories.length - 1} onClick={() => setDetailCategoryIndex(index => Math.min(detailCategories.length - 1, index + 1))}>▶</button></div></Heading><ECostDetailBars category={detailCategory}/><div className={`cost-detail-table ${isProjectCompany ? 'has-trend-column' : ''}`}><div className="cost-detail-head"><span>科目</span><span>成本金额</span><span>占比</span><span>同比</span>{isProjectCompany && <span>趋势</span>}</div>{detailPrimaryItems.map(([name, value, ratio, yoy]) => <React.Fragment key={name}><div className={`cost-detail-row cost-detail-primary ${expandedDetails[name] ? 'expanded' : ''}`}><button type="button" onClick={() => toggleDetail(name)}><span>{expandedDetails[name] ? '⌄' : '›'}</span>{name}</button><span>{value}</span><span>{ratio}</span><ChangeValue value={yoy}/>{isProjectCompany && <button type="button" className="cost-trend-view" onClick={() => setTrendCost(name)}>查看</button>}</div>{expandedDetails[name] && (detailSubItems[name] || []).map(([subName, subValue, subRatio, subYoy]) => <div className="cost-detail-row cost-detail-sub" key={`${name}-${subName}`}><span>{subName}</span><span>{subValue}</span><span>{subRatio}</span><ChangeValue value={subYoy}/>{isProjectCompany && <button type="button" className="cost-trend-view" onClick={() => setTrendCost(subName)}>查看</button>}</div>)}</React.Fragment>)}</div></section>
    <RankingTable rows={rows} title="各项目公司经营成本排名" sort={sort} onSort={onSort} period={period} onPeriodChange={onPeriodChange} formatNumber={formatNumber}/>
  </div>
}
